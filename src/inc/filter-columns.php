    ...
    class Tax_CTP_Filter {
        ...
        public function generate_taxonomy_options( $tax_slug, $parent = '', $level = 0, $selected = null ) {
            ...
            foreach ( $terms as $term ) {
                ...
                $term_count = get_term_post_count_by_type( $term, $tax_slug, $post_type );
                // output each select option line, check against the last $_GET to show the current option selected
                echo '<option value='. $term->slug, $selected == $term->slug ? ' selected="selected"' : '', '>' .$tab. $term->name .' (' . $term_count .')</option>';
